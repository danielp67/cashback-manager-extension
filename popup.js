...
function updateUIBasedOnPreferences(prefs) {
  label.slice(1).forEach((platform) => {
    const row = document.getElementById(`row${platform.charAt(0).toUpperCase() + platform.slice(1)}`);
    if (!prefs[platform]) {
...
chrome.storage.sync.get("cashbackPrefs", ({ cashbackPrefs }) => {
  updateUIBasedOnPreferences(cashbackPrefs || {});
  ...
  fetchData()
    ...
    .then(shops => {
      for (let j = 1; j < label.length; j++) {
        const platform = label[j];
        const row = document.getElementById(`row${platform.charAt(0).toUpperCase() + platform.slice(1)}`);
        if (row.style.display !== 'none') {
          document.getElementById(platform).innerHTML = shops[j] ?
            "<img src='./images/yes.png' width='30px' height='30px'>" :
            "<img src='./images/remove.png' width='30px' height='30px'>";
          if (shops[j] === true) {
            count++;
          }
        }
